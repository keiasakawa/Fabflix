...
@WebServlet(name="FirstLetterServlet",urlPatterns = "/api/first-letter")
public class FirstLetterServlet extends HttpServlet{
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        String query = String.format("select id,title,director,rating,year,price from movies left join ratings on id=movieId where title like ? \n"+
                "order by %s %s, %s %s limit %s",sort1,order1,sort2,order2,limit);
        String pattern = String.format("%s%%",initial);
        System.out.println(initial);
        if(initial.equals("*")){
            pattern = "^[^a-z0-9]";
            query = String.format("select id,title,director,rating,year,price from movies left join ratings on id=movieId where title regexp ? \n"+
                    "order by %s %s, %s %s limit %s",sort1,order1,sort2,order2,limit);
        }
